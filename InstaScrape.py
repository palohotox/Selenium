...
def is_legit(handle, followers_threshold, following_threshold, posts_threshold):
    ...
    try:
        ...
        posts = string.split(" Followers, ")[1].split(" Following, ")[1].split(" Posts")[0].replace(",", "").replace(
            "k", "000")
        if "." in posts:
            posts = posts.replace(".", "")[:-1]

        if int(followers) >= followers_threshold and int(following) >= following_threshold and int(
                posts) >= posts_threshold:
            return True
        else:
            return False
    except:
...
def find_winner(url, tagged_threshold, followers_threshold, following_threshold, posts_threshold):
    ...
    if driver.find_element_by_xpath(
            '//*[@id="react-root"]/section/nav/div[2]/div/div/div[3]/div/div/section/div/button'):
        driver.find_element_by_xpath(
            '//*[@id="react-root"]/section/nav/div[2]/div/div/div[3]/div/div/section/div/button').click()
...
def cli_run():
    ...
    def run():
        ...
        # main function
        find_winner(this_url, this_tagged_threshold, this_followers_threshold, this_following_threshold,
                    this_posts_threshold)
